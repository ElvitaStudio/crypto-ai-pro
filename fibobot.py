...
# Только ЛОНГ. Только 0.618.
TIMEFRAME = '5m'           
CHART_TIMEFRAME = '15m'    
LIMIT_OHLCV = 100          
COINS_TO_SCAN = 60         
VOL_THRESHOLD = 2.0        # Оптимальный порог объема

# ================= СИСТЕМА =================
print("--- Инициализация FIBO-BOT GOLD (Original Strategy)... ---")
...
def generate_chart(symbol, fibo_levels, df_chart):
    try:
        ...
        signal_points[-1] = df['low'].iloc[-1] * 0.995 
        
        ap = mpf.make_addplot(signal_points, type='scatter', markersize=200, marker='^', color='green')

        mpf.plot(df, type='candle', style='yahoo', volume=True, 
                 title=f'\n{symbol} GOLDEN POCKET (0.618)',
                 hlines=hlines_dict,
                 addplot=ap,
...
def analyze_fibo(symbols):
    print(f"\n--- Scan Fibo Gold: {datetime.now().strftime('%H:%M:%S')} ---")
    ...
    for symbol in symbols:
        try:
            ...
            # 1. ГЛАВНОЕ ПРАВИЛО: Тренд должен быть ВВЕРХ
            # (Максимум был ПОЗЖЕ Минимума)
            if idx_high < idx_low: continue 

            diff = swing_high - swing_low
            move_perc = (diff / swing_low) * 100
            
            # 2. ПРАВИЛО ИМПУЛЬСА: Рост должен быть заметным (>2%)
            if move_perc < 2.0: continue

            # 3. ПРАВИЛО ОБЪЕМА: В монете должны быть деньги
            ...
            fibo_618 = swing_high - (diff * 0.618) # Золотое сечение
            
            # СИГНАЛ: Цена спустилась в зону покупки (между 0.5 и 0.618)
            if fibo_618 <= current_price <= fibo_05:
                
                entry = current_price
                stop = swing_low # Классический стоп за начало роста
                take = swing_high # Тейк на перехай
                ...
                # Фильтр: сделка должна стоить риска
                if rr < 1.5: continue
                ...
                msg = (
                    f"🟢`{clean_sym}`\n"
                    f"🚀 Импульс: +{move_perc:.2f}% (Объемы есть)\n\n"
                    f"🎯 **ВХОД (LONG):**\n"
                    f"Цена: {entry}\n"
...
if __name__ == "__main__":
    send_telegram(f"🥇 FIBO-BOT GOLD Запущен!\n(Классическая стратегия: Только Long 0.618)")
