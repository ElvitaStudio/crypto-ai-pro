...
function AIBadge({ signal }: { signal: Signal }) {
  ...
  return (
    <span style={{ fontSize: 11, opacity: 0.75 }}>
      {signal.ai_votes.map(v => v.approved ? '✅' : '❌').join('')} {passed}/{signal.ai_votes.length}
    </span>
  )
}
    ...
    <>
      <div style={styles.card}>
        <div style={styles.row}>
          <span style={{ ...styles.dir, background: signal.direction === 'LONG' ? '#26a17b' : '#e74c3c' }}>
            {signal.direction}
          </span>
...
const styles: Record<string, React.CSSProperties> = {
  card:      { background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: '12px 14px', marginBottom: 10, border: '1px solid rgba(255,255,255,0.08)' },
  row:       { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 },
  dir:       { fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 6, color: '#fff', letterSpacing: 0.5 },
  symbol:    { fontWeight: 700, fontSize: 15, flex: 1 },
  status:    { fontWeight: 700, fontSize: 13 },
  tag:       { fontSize: 11, opacity: 0.6, background: 'rgba(255,255,255,0.07)', padding: '2px 7px', borderRadius: 5 },
  time:      { fontSize: 11, opacity: 0.45, marginLeft: 'auto' },
